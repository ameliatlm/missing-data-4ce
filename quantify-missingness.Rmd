...
```{r setup}
source("R/utils.R")
required_pkgs <- c("tidyverse", "lubridate", "ggridges", "cowplot", "naniar", "DT")
...
df = corr_miss(patient_obs_wide,0.5,0.7)

boxplot(df,ylim=c(0, 1),xlab='Lab Combination',ylab='Spearman Correlation',main='Correlation Between Lab Missing Indicators')




#Generate a heat map for the Spearman correlation between lab missingness at a given time point (in this example, second time point). Legend displays range of Spearman correlation values. 

library(RColorBrewer)

which_mat = 2

heatmap(mat_list[[which_mat]],scale="none",,cexRow=1,cexCol=1,margins=c(8,6))

legend(x="bottomright", legend=c(round(min(mat_list[[which_mat]]),2),round(((max(mat_list[[which_mat]])+min(mat_list[[which_mat]]))/2),2), max(mat_list[[which_mat]])), 
