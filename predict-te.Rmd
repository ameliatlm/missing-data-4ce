...
AKI_results_penn <- theta <- stmfit$theta
lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$AKI)))


Severity_results_penn <- theta <- stmfit$theta
lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$severe)))
...
```{r}
save(topic_diagnostics_penn,topicQuality_penn,TE_results_penn,AKI_results_penn,Severity_results_penn,
