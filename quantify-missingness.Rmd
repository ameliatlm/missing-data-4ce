...
other_vars <- c("patient_num", "days_since_admission", "severity")
...
```{r}
str(imputed_patient_obs)


p_comps = list()
for (vari in labs_to_keep[1:3]){
  vari = "ALT"
  print(vari)
  print(Amelia::compare.density(imputed_patient_obs, var = vari))

}
cowplot::plot_grid(p_comps)
p_comps[[1]]
labs_to_keep
vari = "Albumin"
...
p_comps = list()
par(mfrow=c(3,1))
for (vari in labs_to_keep[1:2]){
  res <- trang_overimpute(imputed_patient_obs, var = vari)
  xplot = res$xplot
  means = res$means
  xlab = res$xlab
  ci.order = res$ci.order
  ylab = res$ylab
  this.ylim = res$this.ylim
  main = res$main
  color = res$color
  lowers = res$lowers
  uppers = res$uppers
  # p_comps[[vari]] <- ~{
    ...
    #   mgp = c(2, 1, 0)
    # )
    plot(xplot[ci.order], means[ci.order], xlab = xlab, 
                     ylab = ylab, ylim = this.ylim, type = "p", main = main, 
                     col = color[ci.order], pch = 19)
    segments(xplot[ci.order], lowers[ci.order], xplot[ci.order], 
             uppers[ci.order], col = color[ci.order])
    # if (legend) {
    #   legend(legpos, legend = c(" 0-.2", ".2-.4", ".4-.6", 
    #                             ".6-.8", ".8-1"), col = spectrum, lty = c(1, 1), 
    #          horiz = TRUE, bty = "n")
    # }
    abline(0, 1)
  # }
