...
during_hosp <- patient_obs_raw %>%
  ...
  left_join(
    demo_raw %>% 
      transmute(patient_num, los = last_discharge_date - admission_date),
    by = 'patient_num') %>% 
  mutate(during = if_else(days_since_admission <= los, 
                          'TE during hospitalization', 
                          'TE after hospitalization')) %>%
  left_join(clin_raw, by = c("patient_num", "days_since_admission")) %>%
  group_by(patient_num, during) %>%
  summarise(thrombo_date = min(calendar_date), 
            .groups = "drop") %>% 
  mutate(week = week(thrombo_date) + ((year(thrombo_date) - 2020) * 52),
         week_name = ymd("2020-01-01") + weeks(week)) 
