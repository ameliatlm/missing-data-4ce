...
#load beta from NWU:
load("/Users/emily/Downloads/beta_matrix2_nwu.Rdata")
...
course <- 
  readr::read_csv(
    file.path(my_dir, "clinicalcourse-upenn.csv"),
  )
...
if ("FEU" %in% colnames(missing_by_patient)) {
  colnames(missing_by_patient)[which(colnames(missing_by_patient)=="FEU")] <- "DDU"
}

missing_by_patient$Troponin = missing_by_patient$Troponin_high +   missing_by_patient$Troponin_normal
...
save(all_evals, beta_mat2, file = "results/beta_matrix_penn.Rdata")
```
...
TE_results_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$TE))[c("estimate","p.value","statistic","method")])

Severity_results_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$severe))[c("estimate","p.value","statistic","method")])

Neuro_results_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$Neuro))[c("estimate","p.value","statistic","method")])

ARDs_results_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$ARDs))[c("estimate","p.value","statistic","method")])
...
TE_results_comb_penn<- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$TE))[c("estimate","p.value","statistic","method")])

Severity_results_comb_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$severe))[c("estimate","p.value","statistic","method")])

Neuro_results_comb_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$Neuro))[c("estimate","p.value","statistic","method")])

ARDs_results_comb_penn <- lapply(1:K, function(x) cor.test(theta[, x], as.numeric(missing_by_patient$ARDs))[c("estimate","p.value","statistic","method")])
...
Severity_boxplot_comb_penn <- topic_df %>% 
  ggplot(aes(x = Severity, y = value)) +
  labs(
    x = "Severity",
    y = "Topic Value"
...
Neuro_boxplot_comb_penn <- topic_df %>% 
  ggplot(aes(x = Neuro, y = value)) +
  labs(
    x = "Neurological Event",
    y = "Topic Value"
...
ARDs_boxplot_comb_penn <- topic_df %>% 
  ggplot(aes(x = ARDs, y = value)) +
  labs(
    x = "Acute Respiratory Distress",
    y = "Topic Value"
...
```{r}
save(topic_diagnostics_penn,topicQuality_penn,TE_results_penn,Severity_results_penn,Neuro_results_penn,ARDs_results_penn,TE_results_comb_penn,Severity_results_comb_penn,Neuro_results_comb_penn,ARDs_results_comb_penn,stmfit_beta_penn,
  file = "results/penn-results-predict-te.Rdata"
)

ggsave('figs/topic_diagnostics_plot_penn.png', topic_diagnostics_plot)
ggsave('figs/TE_boxplot_penn.png', TE_boxplot1_penn)
ggsave('figs/Severity_boxplot_penn.png', Severity_boxplot1_penn)
ggsave('figs/Neuro_boxplot_penn.png', Neuro_boxplot1_penn)
ggsave('figs/ARDs_boxplot_penn.png', ARDs_boxplot1_penn)
ggsave('figs/TE_boxplot_comb_penn.png', TE_boxplot2_penn)
ggsave('figs/Severity_boxplot_comb_penn.png', Severity_boxplot2_penn)
ggsave('figs/Neuro_boxplot_comb_penn.png', Neuro_boxplot2_penn)
ggsave('figs/ARDs_boxplot_comb_penn.png', ARDs_boxplot2_penn)
